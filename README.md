...
Stage3_blastoconsensus.py uses the consensus alleles generated in Stage1.py, rather than phased sequences from Stage2.py, as the reference sequences to build a UBLAST database for inferring hybrid haplotypes and does not annotate hybrid sequence phases, amenable to analyses where you keep the longest or most infromative phased allele for each haplotype pair. This is the method used in Mendez-Reneau et al. (2023), but we did not find significant differences between approaches in our own analyses other than a higher amount of sequence data which provides more informative sites. Using phased alleles may also be more accurate due to representing haplotypes based on phasing rather than consensus assemblies of contigs, which has the potential to incorporate assembly error into inference.
...
This approach may not be appropriate for diploid homoploid hybrids where phased bi-allelic haplotypes (i.e. diploid alleles) are representative of alternative or recombined progenitor genomes either in the same locus or fractionated across loci, as opposed to allopolyploids that retain the full bi-allelic variation (i.e. chromosomes) associated with each progenitor. Assuming homoploidy, some phased haplotype pairs (i.e. in the case of a locus with four bi-allelic sequences associated with two progenitor taxa) may represent a haploid sequence if the locus is "chimeric", or if the hybrid has haplotype pairs represented by atleast two progenitors. If this is the case one could scaffold the associated phases into consensus sequences or choose the longest phase per progenitor as the representative sequence. However, if most loci are not chimeric and simply associated with one progenitor or the other across loci, the latter work-around will lose informative sites for inference and it may be better to retain phased haplotypes.

It is also important to note that phased haplotypes in homoploids may represent recombined loci and that UBLAST annotations for these loci only represent the progenitor taxon that the sequence was more similar to, even if the actual haplotype has SNPS associated with disparate progenitor taxa. High recombination may result in ambiguous placement of hybrid haplotypes on a phylogeny (i.e. subtending progenitor clades with low node support). A possible way to identify and filter recombined sequences or samples would be to inspect the hits.txt ouptut files found in each hybrid sample directory to assess the % identity of each haplotype relative to its the top progenitor hit and only keep sequences or samples meeting dpecific % identity thresholds (i.e. >99% identity to confidently associate a hybrid haplotype with a specific progenitor)
