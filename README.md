...
This approach may not be appropriate for diploid homoploid hybrids where phased bi-allelic haplotypes (i.e. alleles) are representative of alternative progenitor genomes either in the same locus or fractionated across loci, as opposed to allopolyploids that retain the full bi-allelic variation (i.e. chromosomes) associated with each progenitor. Assuming homoploidy, some phased haplotype pairs associated with a progenitor may just represent a single haploid sequence if the locus is "chimeric", or if the hybrid has haplotype pairs represented by atleast two progenitors. If this is the case one could just scaffold the associated phases or choose the longest phase as the representative sequence. However, if most loci are not chimeric and simply associated with one progenitor or the other across loci, the latter work-around might lose informative sites for inference and it may be better to retain phased haplotypes while being aware that they may result in identical homozygous haplotypes due to actually representing a single haploid sequence (per progenitor) if the locus is truly chimeric. It is also important to note that phased haplotypes in homoploids may represent recombined loci and that UBLAST annotations for these loci only represent the progenitor taxon that the sequence was more similar to, even if the actual haplotype has SNPS associated with disparate progenitor taxa. High recombination may result in ambiguous placement of hybrid haplotypes on a phylogeny (i.e. subtending progenitor clades with low node support). A possible way to identify and filter recombined sequences or samples would be to inspect the hits.txt ouptut files found in each hybrid sample directory to assess the % identity of each haplotype relative to its the top prognitor hit and only keep sequences or samplesmkeeting certain % identity thresholds (i.e. >99% identity to confidently associate a hybrid haplotype with a specific progeniter)
